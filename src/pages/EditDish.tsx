...
export default function EditDish() {
  ...
  const fetchInitialData = async () => {
    ...
    let dishRes = await supabase.from("dishes").select().match({ id: dishId });

    if (dishRes.data?.length) setInitialValues(dishRes.data[0]);
  ...
  const handleDishDelete = () => {
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        await supabase.from("dishes").delete().match({ id: dishId });
  ...
  return (
    <Page>
      <PageTitle showBackBtn text="Edit Dish" />
        ...
        {({ values, setFieldValue, isSubmitting }) => (
          <Form>
            <Row gutter={[20, 20]}>
              ...
              <Col span={24} className={classes.inputWrapper}>
                <Typography.Text>Lunch Price</Typography.Text>
